...
class ConfigDialog extends Dialog {
    ...
    this.urlLabel.textAlignment = TextAlignment.Right | TextAlignment.VertCenter;
    ...
    this.tempLabel.textAlignment = TextAlignment.Right | TextAlignment.VertCenter;

    this.tokensLabel = new Label(this);
    this.tokensLabel.text = "Max Tokens:";
    this.tokensLabel.textAlignment = TextAlignment.Right | TextAlignment.VertCenter;
    ...
    this.visionMaxDimLabel.textAlignment = TextAlignment.Right | TextAlignment.VertCenter;
    ...
    this.promptLabel.textAlignment = TextAlignment.Left | TextAlignment.VertCenter;
    ...
    this.helpButton.onClick = function() {
	    if (!Dialog.browseScriptDocumentation( TITLE, "LLM Assistant" )) {
            console.writeln("Documentation not found installed locally.");
            Dialog.openBrowser( HELP_URL );
        }
	    return;
    }.bind(this);
    ...
    buttonsSizer.add(this.helpButton); // HELP
    ...
    this.onReset = function() {
        ...
        this.config.url = defaultConfig.url;
        this.config.apiKey = defaultConfig.apiKey;
        this.config.model = defaultConfig.model;
        this.config.systemPrompt = defaultConfig.systemPrompt;
        this.config.temperature = defaultConfig.temperature;
        this.config.maxTokens = defaultConfig.maxTokens;
        this.config.enableVision = defaultConfig.enableVision;
        this.config.visionMaxDimension = defaultConfig.visionMaxDimension;

        this.updateUIFromConfig();
        console.writeln("LLM Assistant configuration has been reset to defaults.");
    };
    ...
    this.isFirstRun = function() {
        Settings.read(this.hasBeenConfiguredKey, DataType.Boolean);
        return !Settings.lastReadOK;
    };

    this.setHasBeenConfigured = function() {
        Settings.write(this.hasBeenConfiguredKey, DataType.Boolean, true);
    };
...
class Configuration {
    ...
    this.load = function() {
        let value;
        value = Settings.read(SETTINGS_URL, DataType.String);
        if (Settings.lastReadOK) this.url = value;
        value = Settings.read(SETTINGS_APIKEY, DataType.String);
        if (Settings.lastReadOK) this.apiKey = value;
        value = Settings.read(SETTINGS_MODEL, DataType.String);
        if (Settings.lastReadOK) this.model = value;
        value = Settings.read(SETTINGS_PROMPT, DataType.String);
        if (Settings.lastReadOK) this.systemPrompt = value;
        value = Settings.read(SETTINGS_TEMP, DataType.Double);
        if (Settings.lastReadOK) this.temperature = value;
        value = Settings.read(SETTINGS_TOKENS, DataType.Int32);
        if (Settings.lastReadOK) this.maxTokens = value;
        value = Settings.read(SETTINGS_VISION_ENABLED, DataType.Boolean);
        if (Settings.lastReadOK) this.enableVision = value;
        value = Settings.read(SETTINGS_VISION_MAX_DIM, DataType.Int32);
        if (Settings.lastReadOK) this.visionMaxDimension = value;
    };

    this.save = function() {
        Settings.write(SETTINGS_URL, DataType.String, this.url);
        Settings.write(SETTINGS_APIKEY, DataType.String, this.apiKey);
        Settings.write(SETTINGS_MODEL, DataType.String, this.model);
        Settings.write(SETTINGS_PROMPT, DataType.String, this.systemPrompt);
        Settings.write(SETTINGS_TEMP, DataType.Double, this.temperature);
        Settings.write(SETTINGS_TOKENS, DataType.Int32, this.maxTokens);
        Settings.write(SETTINGS_VISION_ENABLED, DataType.Boolean, this.enableVision);
        Settings.write(SETTINGS_VISION_MAX_DIM, DataType.Int32, this.visionMaxDimension);
    };
