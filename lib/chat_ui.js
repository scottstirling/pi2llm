...
function pi2llmChatDialog(config) {
    ...
    this.historyBox.text = "<html><p style='color:#888;'><i>Select an image from the dropdown and click 'Analyze' to begin.</i></p></html>";
    ...
    this.inputBox.onKeyPress = function(key, modifiers) {

        // Check for Ctrl+Enter to send the message
        if (key === 13 && modifiers === KeyModifier_Control) {
            if (this.sendButton.enabled) {
                this.sendButton.onClick();
            }
            key.accepted = true; // Tell the system we handled this key press
        } else if (key == 13) {
            this.inputBox.text += "\n";
            key.accepted = true;
        }
    ...
    // -- Bottom Button Panel --
    // "NEW CHAT" BUTTON
    // TODO: what to do about keyboard focus going to newChatButton after the user types into the input area.
    ...
    this.resetChat = function() {
        ...
        // this.sendButton.defaultButton = true; // v2
    ...
    switch (role) {
        ...
        // Add a <p> tag at the start for a newline and a <div> wrapper for structure.
        let newhtml = "<br><div style=\"margin-bottom: 10px; color: " + color + ";\"><b>" + displayName + ": </b>" +
               "<div style=\"padding-left: 15px;\">" + formattedContent + "</div></div>";
    ...
    this.runInitialAnalysis = function() {
        // 1. Get the window object from the user's selection in the ViewList.
        let selectedView = this.viewList.currentView;
        if (selectedView.isNull) {
            new MessageBox("Please select a valid image from the dropdown.", "PI2LLM Assistant").execute();
            return;
        }
        ...
        this.historyBox.text = "<p style='color:#888;'><i>Extracting profile for " + selectedView.id + " ... Please wait.</i></p>";

        // Let the UI update before doing heavy work
        processEvents();

        // 3. Call our focused data extraction function.
        let imageProfileDTO = createFocusedImageProfile(selectedWindow);

        this.chatHistory.push({ role: "system", content: this.config.systemPrompt });
        this.chatHistory.push({ role: "user", content: JSON.stringify(imageProfileDTO) });

        this.addMessageToUI("system", "Analysis complete. Sending data for " + imageProfileDTO +
                     " images to LLM, with a focus on " + imageProfileDTO.activeViewId + ".");
        this.sendRequest();
    };
