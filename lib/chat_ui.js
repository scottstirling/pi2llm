...
function pi2llmChatDialog(config) {
    ...
    this.sendImageCheckBox.toolTip = "<p>Prepares and sends a JPG version of the selected image with your request.</p>";
    ...
    /**
     * Global key press handler for the entire dialog.
     */
    ...
    this.settingsButton.onClick = function() {
       ...
       // After the settings dialog is closed, immediately call
        // helper function to refresh the main chat UI's state.
    ...
    /**
     * Updates the visibility and state of UI components based on the
     * current state of the this.config object.
     */
    this.updateUIFromConfig = function() {
        console.writeln("Updating chat UI from configuration...");
        this.sendImageCheckBox.visible = this.config.enableVision;
    };
    ...
    this.onSuccess = function(response) {
        ...
        if (Array.isArray(lastMessage.content)) {
            lastMessage.content = JSON.parse(lastMessage.content[0].text);
                ...
                this.inputBox.enabled = true;
                this.inputBox.focused = true;
                this.sendImageCheckBox.enabled = true;
                // Re-enable the analysis controls and refresh the image list
                this.viewList.getAll();
                this.viewList.enabled = true;
                this.analyzeButton.enabled = !this.viewList.currentView.isNull;
    ...
    // --- Welcome Logic ---
    // This is the new, non-intrusive first-run experience.
    if (this.config.isFirstRun()) {
        this.historyBox.text = "<p style='font-size:14px; color:#333; margin:10px;'>" +
            "<b>Welcome to the LLM Assistant for PixInsight!</b></p>";
        // Mark them as "configured" now, so this message doesn't appear again.
        this.config.setHasBeenConfigured();
    } else {
        // The normal startup message for returning users.
        this.historyBox.text = "<p style='color:#888;'><i>Select an image from the dropdown and click 'Analyze'.</i></p>";
    }
