...
class ImagePreparer {
    ...
    constructor(view) {
        ...
        this.view = view;
        this.newViewBaseId = "_llmscript";
        this.tmpJpgFilename = this.newViewBaseId + this.view.id + ".jpg";
        // TODO add option to produce and send other visual LLM supported file types such as png and webp
    }
    prepare(maxDimension) {
        ...
        let workingImage = this.cloneView(this.view, this.newViewBaseId, this.maxDimension);
        let workingView = workingImage.mainView;

        // DEBUG
        console.writeln("workingImage: "+ workingImage);
        console.writeln("workingView: "+ workingView);
        console.writeln("tmpJpgFilename: "+ this.tmpJpgFilename);
        ...
        let outputFilePath = File.systemTempDirectory + '/' + this.tmpJpgFilename;

       if (jpgInstance.create(outputFilePath)) {
	   if (jpgInstance.writeImage(workingView.image)) {
        ...
        // close temporary view used to create resized image
        workingImage.forceClose();
 ...
 */
cloneView(sourceView, newId, maxDimension) {
    ...
    var newWindow = new ImageWindow(1,1,1,32,true,false,newId);
    ...
    newView.beginProcess(UndoFlag.NoSwapFile);

    // Assign the image data from the source view to the new view.
    newView.image.assign(sourceView.image);

    // End the process.
    newView.endProcess();

        // --- 2. Resize if Necessary ---
        let width = newView.image.width;
        let height = newView.image.height;
        ...
        if (width > maxDimension || height > maxDimension) {
            ...
            newView.image.resample( newWidth, newHeight, ResizeMode_AbsolutePixels);
            newView.endProcess();

            console.writeln("Resized to ", newView.image.width, " x ", newView.image.height);
        }
       ...
       // newWindow.zoomToOptimalFit(); // uncomment for DEBUG
        ...
        return newWindow; // return window so we can close it and its view later
    }
}
